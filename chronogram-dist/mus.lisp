((LANGUAGE . "mus")
 (CALENDARS
  (GREGORIAN
   (MONTHS
    (FORMAT
     (WIDE
      (DEFAULT (12 . "Rvfo Rakko") (11 . "Ehole") (10 . "Otowoskv Rakko")
       (9 . "Otowoskuce") (8 . "Hiyo Rakko") (7 . "Hiyuce") (6 . "Kvco Hvse")
       (5 . "Ke Hvse") (4 . "Tasahce Rakko") (3 . "Tasahcuce")
       (2 . "Hotvle Hvse") (1 . "Rvfo Cuse")))))
   (DAYS
    (FORMAT
     (WIDE
      (DEFAULT (SAT . "Nettv Cakcuse") (FRI . "Nak Okkoskv Nettv")
       (THU . "Ennvrkvpv Enhvyvtke") (WED . "Ennvrkvpv")
       (TUE . "Enhvteceskv Enhvyvtke") (MON . "Enhvteceskv")
       (SUN . "Nettv Cako")))))
   (DATE-FORMATS (SHORT (PATTERN . "M/d/yy")) (MEDIUM (PATTERN . "MMM d, y"))
    (LONG (PATTERN . "MMMM d, y")) (FULL (PATTERN . "EEEE, MMMM d, y")))
   (TIME-FORMATS (SHORT (PATTERN . "h:mm a")) (MEDIUM (PATTERN . "h:mm:ss a"))
    (LONG (PATTERN . "h:mm:ss a z")) (FULL (PATTERN . "h:mm:ss a zzzz"))))
  (GENERIC
   (DATE-FORMATS (SHORT (PATTERN . "M/d/y GGGGG"))
    (MEDIUM (PATTERN . "MMM d, y G")) (LONG (PATTERN . "MMMM d, y G"))
    (FULL (PATTERN . "EEEE, MMMM d, y G"))))))
